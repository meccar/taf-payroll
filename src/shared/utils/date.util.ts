...
export class DateUtils {
  ...
  private static readonly APP_TIMEZONE =
  ...
  static getDateStringWithFormat(
    ...
    const format =
      input.format ?? process.env.DATE_FORMAT ?? this.DEFAULT_DATE_FORMAT;

    return DateTime.fromJSDate(date, { zone: 'utc' })
      .setZone(this.APP_TIMEZONE)
      .toFormat(format);
  }

  static getISODateString(): string {
    return DateTime.fromJSDate(this.getJSDate(), { zone: 'utc' })
      .setZone(this.APP_TIMEZONE)
      .toISO();
  }
